...
export async function analyzeLecture(base64Data: string, mimeType: string, stats: UserStats): Promise<LectureAnalysis> {
  ...
  const prompt = `أنت خبير في تبسيط المحاضرات لطلبة الطب.
اشرح المحاضرة المرفقة بالتفصيل بـ (${dialectMap[stats.dialect] || 'اللهجة العراقية'}).

الشروط:
1. استخدم لغة عامية طبية بسيطة ومفهومة.
2. اذكر الأسماء التجارية للأدوية.
3. ابدأ بمقدمة 'academic' عن الفسلجة/التشريح الطبيعي للموضوع.
4. نظام البطاقات: 'academic' للعلم، 'clinical' للسريريات، 'red_flag' للطوارئ.
5. ميزة المراجعة: لخص المحاضرة السابقة بـ 6 أسطر.

المحاضرة السابقة:
${lastLecture ? `العنوان: ${lastLecture.title}\nالملخص: ${lastLecture.summary}` : 'لا توجد.'}
